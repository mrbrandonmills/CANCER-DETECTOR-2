...
class _DeepResearchScreenState extends State<DeepResearchScreen> {
  ...
  final PdfService _pdfService = PdfService(); // Keep as fallback
  DeepResearchJob? _currentJob;
  String? _currentJobId; // Store job ID for PDF endpoint
  Timer? _pollTimer;
  ...
  Future<void> _startDeepResearch() async {
    try {
      ...
      setState(() {
        _currentJobId = jobId; // Store for PDF endpoint
      });
  ...
  /// Download PDF from server and share
  /// Server-side generation eliminates iOS memory crashes from client-side dart_pdf
  Future<void> _sharePdfReport() async {
    if (_currentJob?.result == null || _currentJobId == null) return;
    ...
    try {
      // Download PDF from server (no more iOS memory crashes!)
      final pdfBytes = await _apiService.downloadDeepResearchPdf(_currentJobId!);

      // Save to temp file for sharing
      final tempDir = await getTemporaryDirectory();
      final productName = _currentJob!.result!.productName;
      final safeName = productName.replaceAll(RegExp(r'[^\w\s-]'), '').replaceAll(RegExp(r'\s+'), '_');
      final file = File('${tempDir.path}/TrueCancer_Report_$safeName.pdf');
  ...
  /// Download PDF from server and print directly
  Future<void> _printPdfReport() async {
    ...
    try {
      ...
      // Print directly using the printing package
      await Printing.layoutPdf(
        onLayout: (format) async => Uint8List.fromList(pdfBytes),
        name: 'TrueCancer_Report_${_currentJob!.result!.productName}',
      );
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
